...
def test_search(api, api_key, requests_mock):
    ...
    request_record = requests_mock.request_history.pop()
    assert request_record.method == 'GET'
    assert request_record.url == f'{api.kb_api_url}/name_search?q=bee'
    assert request_record.headers['Content-Type'] == 'application/json'
    assert request_record.headers['Api-Key'] == api_key
    ...
    request_record = requests_mock.request_history.pop()
    assert request_record.url == f'{api.kb_api_url}/name_search?q=vcela&limit=1&language=cz'
    # check bundle
    ...
    requests_mock.get(f'{api.kb_api_url}/plants/name_search', json=plant_api_res)
    api.search('aloe')
    request_record = requests_mock.request_history.pop()
    assert request_record.url == f'{api.kb_api_url}/plants/name_search?q=aloe'
    # fails on bad kb_type
    with pytest.raises(ValueError):
        api.search('downy', kb_type='disease')
    requests_mock.get(f'{api.kb_api_url}/diseases/name_search', json=plant_api_res)
    api.search('downy', kb_type='diseases')
    request_record = requests_mock.request_history.pop()
    assert request_record.url == f'{api.kb_api_url}/diseases/name_search?q=downy'
    with pytest.raises(ValueError):
...
def test_get_kb_detail(api, api_key, requests_mock):
    ...
    request_record = requests_mock.request_history.pop()
    assert request_record.url == f'{api.kb_api_url}/{access_token}?details=rank,gbif_id&language=cs'

    # check bundle
    api = PlantApi(api_key=api_key)
    requests_mock.get(
        f'{api.kb_api_url}/plants/{access_token}',
        json={'gbif_id': 2777724, 'rank': 'species', 'language': 'en', 'name': 'Aloe vera'},
    )
    response = api.get_kb_detail(access_token, 'rank,gbif_id')
    request_record = requests_mock.request_history.pop()
    assert request_record.url == f'{api.kb_api_url}/plants/{access_token}?details=rank,gbif_id'
    assert response == {'gbif_id': 2777724, 'rank': 'species', 'language': 'en', 'name': 'Aloe vera'}

    requests_mock.get(
        f'{api.kb_api_url}/diseases/{access_token}',
        json={'url': 'https://en.wikipedia.org/wiki/Downy_mildew', 'language': 'en', 'name': 'downy mildew'},
    )
    response = api.get_kb_detail(access_token, 'url', kb_type='diseases')
    request_record = requests_mock.request_history.pop()
    assert request_record.url == f'{api.kb_api_url}/diseases/{access_token}?details=url'
